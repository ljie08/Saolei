...
//    self.view.backgroundColor = [self hexStringToColor:@"#ebead8"];
//    self.view.backgroundColor = [UIColor colorWithRed:215 green:220 blue:179 alpha:1];
...
- (void)normalClickStyleWithSender:(SaoleiChessView *)sender {
    ...
    if (sender.enabled) {
        ...
        if (sender.clickKind == SaoleiUserClickKindNormal) {
             ...
             */
            if (sender.isLei) {
                [sender setBackgroundImage:[UIImage imageNamed:@"lei"] forState:(UIControlStateDisabled)];
                ...
                if (sender.numberOfLei) {
                    NSString *string = [NSString stringWithFormat:@"num_%ld",sender.numberOfLei];
                    
                    [sender setBackgroundImage:[UIImage imageNamed:string] forState:(UIControlStateDisabled)];
                    
                    sender.enabled = NO;
                }else {
                    [sender setBackgroundImage:[UIImage imageNamed:@"fk_selected"] forState:(UIControlStateDisabled)];
